...
open_local_device(struct doca_pci_bdf *desired_addr, struct app_state *state)
{
	...
	for (it = 0; it < dev_count; ++it) {
		if (doca_devinfo_property_get(devlist[it], DOCA_DEVINFO_PROPERTY_PCI_ADDR, (uint8_t *)&dev_addr,
		    ...
		    desired_addr->bus == dev_addr.bus && desired_addr->device == dev_addr.device && desired_addr->function == dev_addr.function) {
...
doca_error_t
create_core_objects(struct app_state *state, char *mmap_name, char *inv_name)
{
	doca_error_t res;

	res = doca_mmap_create(mmap_name, &state->mmap);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to create mmap: %s", doca_get_error_string(res));
		return res;
	}

	res = doca_buf_inventory_create(inv_name, 2, DOCA_BUF_EXTENSION_NONE, &state->buf_inv);
	if (res != DOCA_SUCCESS) {
...
doca_error_t
init_core_objects_sender(struct app_state *state, char *mmap_name)
{
	doca_error_t res;
